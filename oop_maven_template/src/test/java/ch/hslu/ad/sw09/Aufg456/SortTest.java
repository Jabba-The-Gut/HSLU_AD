...
import ch.hslu.ad.sw09.Aufg456.Sort;
...
public class SortTest {
	...
	/**
	 * Test method for {@link ch.hslu.ad.sw09.Aufg456.Sort#insertionSort(int[])}.
	 */
	@Test
	...
	public void testInsertionSortReverseSorted() throws Exception {
		...
		Sort.insertionSort(sorted);
		long finish = System.currentTimeMillis();
		LOG.info("Insertion-Sort: Verkehrt-Sortiertes Array: " + (finish - start));
		Assert.assertArrayEquals(sorted2, sorted);
	}

	/**
	 * Test method for {@link ch.hslu.ad.sw09.Aufg456.Sort#selectionSort(int[])}.
	 */
	@Test
	...
	@Test
	public void testBubbleSortDoubleValues() throws Exception {
		int[] unsorted = { 6, 5, 1, 12, 9, 6, 1, 3 };
		Sort.bubbleSort(unsorted);
		Assert.assertArrayEquals(new int[] { 1, 1, 3, 5, 6, 6, 9, 12 }, unsorted);
	}
	...
	@Test
	public void testBubbleSortSorted() throws Exception {
		int[] sorted = new int[10000];
		for (int i = 0; i < 10000; i++) {
			sorted[i] = i;
		}
		...
		long finish = System.currentTimeMillis();
		LOG.info("Bubble-Sort: Sortiertes Array: " + (finish - start));
		Assert.assertArrayEquals(sorted2, sorted);
	}

	@Test
	public void testBubbleSortReverseSorted() throws Exception {
		int[] sorted = new int[10000];
		int value = sorted.length - 1;
		for (int i = 0; i < 10000; i++) {
			sorted[i] = value;
		...
		Sort.bubbleSort(sorted);
		long finish = System.currentTimeMillis();
		LOG.info("Bubble-Sort: Verkehrt-Sortiertes Array: " + (finish - start));
		Assert.assertArrayEquals(sorted2, sorted);
	}
}
