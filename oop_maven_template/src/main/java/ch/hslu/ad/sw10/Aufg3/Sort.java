...
public class Sort {
	...
	public static final void quickInsertionSort(final char[] a, final int TRESHOLD) {
		...
		if ((left - (up - 1)) < TRESHOLD) {
			...
			quickInsertionSort(a, left, (up - 1), TRESHOLD); // linke Hälfte
		}
		if (((up + 1) - right) < TRESHOLD) {
			Sort.insertionSort(a);
		} else {
			if ((up + 1) < right)
				quickInsertionSort(a, (up + 1), right, TRESHOLD); // rechte Hälfte, ohne T’Elt.}
		}
	}
	
	public static final void quickInsertionSort(final char[] a,final int left, final int right, final int TRESHOLD) {
